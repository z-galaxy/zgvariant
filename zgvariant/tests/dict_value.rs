...
fn dict_value() {
    ...
    let gv_ser_value_encoded =
        zgvariant::to_bytes(ctxt, &zgvariant::as_value::Serialize(&map)).unwrap();
    let gv_value_encoded = to_bytes(ctxt, &zgvariant::Value::new(map)).unwrap();
    ...
    // Same derive-namespace coverage, but through the `#[zgvariant(...)]` attribute spelling
    // rather than the legacy `#[zvariant(...)]` one.
    #[derive(DeserializeDict, SerializeDict, Type, PartialEq, Debug, Default)]
    #[zgvariant(signature = "dict")]
    struct TestZgvariantNamespace {
        process_id: Option<u32>,
        group_id: Option<u32>,
    }
    let decoded: TestZgvariantNamespace = encoded.deserialize().unwrap().0;
    assert_eq!(decoded.process_id, Some(42));
    ...
    // An empty dict has no entries and thus no bytes at all in the GVariant format (no length
    // prefix, unlike D-Bus).
    let data = to_bytes(ctxt, &TestEmpty::default()).unwrap();

    assert_eq!(data.bytes(), &[] as &[u8]);
}
