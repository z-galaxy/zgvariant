use std::vec;
use zgvariant::{Array, LE, Type, Value, serialized::Context, to_bytes};
...
fn array_value() {
    ...
    let at_decoded = gv_encoded.deserialize::<Vec<u64>>().unwrap().0;
    assert_eq!(at_decoded.len(), 0);
    ...
    assert_eq!(<[u64; 0]>::SIGNATURE, &zgvariant::Signature::U8);
    let array: [u64; 0] = [];
