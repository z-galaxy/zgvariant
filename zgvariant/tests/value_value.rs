...
use serde_json::json;
use zgvariant::{BE, LE, OwnedValue, Value, serialized::Context, to_bytes};
...
fn value_value() {
    ...
    let value = zgvariant::Value::new("variant-value");
    let inner = zgvariant::StructureBuilder::new()
        .add_field("value1".to_string())
        .add_field("value2")
        .append_field(zgvariant::Value::new(value)) // let's try to get a variant
        .build()
        .unwrap()
        .try_into()
        .unwrap();
