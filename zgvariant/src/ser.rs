...
///
/// ```
/// use zgvariant::{serialized::Context, serialized_size, LE};
///
/// let ctxt = Context::new(LE, 0);
/// let len = serialized_size(ctxt, "hello world").unwrap();
/// assert_eq!(*len, 12);
///
/// let len = serialized_size(ctxt, &(1u64, 2u64)).unwrap();
/// assert_eq!(*len, 16);
/// ```
    ...
    T: ?Sized + Serialize + DynamicType,
{
    ...
    let mut ser = Serializer::<NullWriteSeek>::new(&signature, &mut null, ctxt)?;
    value.serialize(&mut ser)?;

    Ok(Size::new(ser.0.bytes_written, ctxt))
}
...
///
/// ```
/// use zgvariant::{serialized::{Context, Data}, to_writer, LE};
///
/// let ctxt = Context::new(LE, 0);
...
/// assert_eq!(value, 42);
/// ```
pub fn to_writer<W, T>(writer: &mut W, ctxt: Context, value: &T) -> Result<Written>
where
    W: Write + Seek,
    T: ?Sized + Serialize + DynamicType,
{
...
pub fn to_bytes<T>(ctxt: Context, value: &T) -> Result<Data<'static>>
where
    T: ?Sized + Serialize + DynamicType,
{
...
/// [`DynamicType`].
pub fn to_writer_for_signature<W, S, T>(
    writer: &mut W,
    ctxt: Context,
    signature: S,
    value: &T,
    ...
    T: ?Sized + Serialize,
{
    ...
    let mut ser = Serializer::<W>::new(&signature, writer, ctxt)?;
    value.serialize(&mut ser)?;

    Ok(Written::new(ser.0.bytes_written, ctxt))
}
...
/// use this function.
pub fn to_bytes_for_signature<S, T>(ctxt: Context, signature: S, value: &T) -> Result<Data<'static>>
where
    S: TryInto<Signature>,
    S::Error: Into<Error>,
    T: ?Sized + Serialize,
{
    ...
    Ok(Data::new(cursor.into_inner(), ctxt))
}

/// Context for our serializer and provides shared functionality.
...
pub(crate) struct Serializer<'ser, W>(pub(crate) SerializerCommon<'ser, W>);
...
macro_rules! serialize_basic {
    ($method:ident($type:ty) $write_method:ident) => {
        fn $method(self, v: $type) -> Result<()> {
            ...
            self.0
                .$write_method(self.0.ctxt.endian(), v)
                .map_err(|e| Error::InputOutput(e.into()))
        }
    };
}
    ...
    W: Write + Seek,
{
    ...
    serialize_basic!(serialize_u8(u8) write_u8);
    ...
    fn serialize_i32(self, v: i32) -> Result<()> {
        // `Signature::Fd` only exists on Unix (it's the D-Bus file-descriptor-index type, which
        // this crate doesn't support); route it to an error instead of silently mis-encoding it.
        #[cfg(unix)]
    ...
    W: Write + Seek,
{
        ...
        T: ?Sized + Serialize,
    {
        ...
        let mut ser = Serializer(SerializerCommon::<W> {
