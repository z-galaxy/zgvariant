...
impl TryFrom<Signature> for StructureSeed<'static> {
    type Error = zgvariant::Error;

    fn try_from(signature: Signature) -> Result<Self, zgvariant::Error> {
        if !matches!(signature, Signature::Structure(_)) {
            return Err(zgvariant::Error::IncorrectType);
        }
...
impl<'a> DynamicDeserialize<'a> for Structure<'a> {
    ...
    fn deserializer_for_signature(signature: &Signature) -> zgvariant::Result<Self::Deserializer> {
