...
use crate::{
    Array, Basic, Dict, DynamicType, Maybe, ObjectPath, OwnedValue, Signature, Str, Structure,
    StructureBuilder, Type, array_display_fmt, dict_display_fmt, maybe_display_fmt,
    structure_display_fmt, utils::*,
};

/// A generic container, in the form of an enum that holds exactly one value of any of the other
/// types.
///
/// Note that this type corresponds to the GVariant `v` (variant) type, and as such, its encoding
/// is not the same as that of the enclosed value.
///
/// # Examples
///
/// ```
...
impl<'a> Value<'a> {
    ...
    ///
    /// ```
    /// use zgvariant::{Error, Result, Value};
    ///
...
impl Clone for Value<'_> {
    fn clone(&self) -> Self {
        self.try_clone().expect("cloning a `Value` can't fail")
    }
}
