...
/// Error type used by zgvariant API.
#[derive(Debug)]
#[non_exhaustive]
pub enum Error {
    ...
    /// Attempted to create an empty structure (which is not allowed by the GVariant
    /// specification).
    EmptyStructure,
    /// Invalid object path.
    InvalidObjectPath,
...
impl PartialEq for Error {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            (Error::UnsupportedType(sig1), Error::UnsupportedType(sig2)) => sig1 == sig2,
            (
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Error::UnsupportedType(sig) => {
                write!(f, "Type `{sig}` is not supported by the GVariant format")
            }
...
impl Clone for Error {
    fn clone(&self) -> Self {
        match self {
            ...
            Error::UnsupportedType(sig) => Error::UnsupportedType(sig.clone()),
...
/// Alias for a `Result` with the error type `zgvariant::Error`.
